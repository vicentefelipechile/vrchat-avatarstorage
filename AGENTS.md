...
- **Rate Limiting:** Cloudflare native Rate Limiting bindings — `RL_STRICT` (1/60s), `RL_MEDIUM` (100/60s), `RL_GLOBAL` (500/60s), `RL_LOGIN` (10/60s). Configured in `wrangler.jsonc`. `POST ...?validate_only=true` (avatar/asset/clothes/resource/blog preflight) is routed to `RL_MEDIUM` to avoid consuming `RL_STRICT` budget (UploadView does 2 POSTs <1s). Local dev (`localhost`/`127.0.0.1` exact hostname) bypasses rate limiting entirely via `isLocalRequest` strict check.
...
| **Seed DB**              | `npm run seed [--count=N] [--no-r2] [--keep] [--remote]` (default 200, weighted 65/15/20) — see [Testing](#testing) |
...
- `npm run seed` is **working** via `tsx src/test/setup/populate.ts` (`src/test/setup/`). It seeds D1 + R2 locally (default 200 resources, weighted 65% avatars / 15% assets / ~20% clothes) with production-like variety. Use `npm run seed -- --count=30` for a fast run, `--count=200` for full scale, `--no-r2` for DB-only, `--keep` to preserve existing data, `--remote` for prod (with care). Clothes items get 1..8 `clothing_type` values via `clothes_clothing_types` junction (weighted 1-2 common, 3-4 occasional, 5-8 rare, max 8 enforced by `ClothesMetaSchema`). Standard values only: avatar genders `male`/`female`/`both` (excludes internal `undefined` and legacy `androgynous`), avatar/assets/clothes `other` kept as rare exceptional fallback. R2 local upload uses `getPlatformProxy` with binding names `BUCKET`/`MEDIA_BUCKET` (not bucket names). The sample images in `public/test/` are the fallback when `.wrangler/img-seed/` is empty.
...
Clothes `clothing_type` is **multi-value 1..8** (`src/validators.ts:ClothesMetaSchema` array, max 8, unique, junction `clothes_clothing_types` via `migrations/0024_clothes_multi_type.sql`). Create/update payload is `clothing_type: string[]`; list/detail return `clothing_type: string[]` (no legacy single string). Filtering is `OR` via `EXISTS(SELECT 1 FROM clothes_clothing_types WHERE ... IN (...))` (`src/repositories/clothes-repository.ts:103`); empty filter returns all. Card badge shows `first +N` (`src/frontend/views/ClothesView.ts:89`), detail `ItemView` renders all types as chips (`src/frontend/views/ItemView.ts:94`), upload/edit use chip-toggle checkboxes (`UploadView.ts`/`EditResourceView.ts`). Seed generates 1..8 weighted types via the junction.
...
src/
  ...
  http/
    ...
    routes/               # Thin HTTP handlers (route layer) — one file per API domain
      ...
      downloads.ts        favorites.ts  feed.ts
...
- `rateLimit` (legacy): KV-based rate limiter in `src/http/middleware/rate-limit.ts`. Key format: `rate_limit:{prefix}:{key}`. Most endpoints now use the **Cloudflare native Rate Limiting bindings** (`RL_STRICT`, `RL_MEDIUM`, `RL_GLOBAL`, `RL_LOGIN`) configured directly in `wrangler.jsonc` and applied in `src/http/rate-limits.ts` via `rateLimit({ binding: c.env.RL_*, keyPrefix: '...' })`. `isLocalRequest` checks exact hostname (`localhost`/`127.0.0.1`/`::1` via `Host` header + `new URL(...).hostname`) to bypass limits only in `wrangler dev`; substring checks are forbidden (would be exploitable via `?q=localhost`). `isValidationRequest` routes `validate_only=true` preflights to `RL_MEDIUM` so `UploadView` 2× POST in <1s doesn't burn `RL_STRICT`.
...
**Video playback (`ItemView`):** a video in the gallery renders as its animated GIF poster with a play badge (`.gallery-video-badge`), not an inline `<video>`. Clicking opens the shared lightbox, which mounts a native `<video>` element (browser-native controls) on demand over the `?format=video` MP4 (poster = `?format=gif`). A custom download button (`.lightbox-video-download`) is overlaid top-right; its click triggers a real file download (`<a download>`) of the MP4, named from the resource title. The `<video>` is paused and torn down on every slide change and on close so a paged-past video never keeps playing.
