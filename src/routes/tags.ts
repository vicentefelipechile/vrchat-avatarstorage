...
tags.get('/', async (c) => {
	...
	try {
		let stmt;
		if (query) {
			stmt = c.env.DB.prepare('SELECT * FROM tags WHERE name LIKE ? ORDER BY name ASC LIMIT 20').bind(`%${query}%`);
		} else {
...
tags.post('/', async (c) => {
	...
	try {
		...
		const result = await c.env.DB.prepare('INSERT INTO tags (name) VALUES (?) RETURNING *').bind(name).first<Tag>();
