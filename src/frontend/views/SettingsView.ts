...
export async function settingsAfter(_ctx: RouteContext): Promise<void> {
	...
	anonCheckbox.addEventListener('change', () => {
		if (anonCheckbox.checked) {
			...
			if (f && f.type.startsWith('image/')) {
				...
				// blob: URLs are generated by the browser and cannot be controlled by the file content
				if (blobUrl.startsWith('blob:')) imgPreview.src = blobUrl;
				else imgPreview.src = originalAvatarUrl;
			} else {
	...
	avatarInput.addEventListener('change', (e) => {
		...
		// Explicit blob: prefix check so CodeQL knows this is not an attacker-controlled string/HTML
