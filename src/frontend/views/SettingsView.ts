...
import { renderTurnstile, resizeImage, showToast, loadingBtn, mediaUrl, metaLabel, safeHttpUrl } from '../lib/utils';
...
export async function settingsView(_ctx: RouteContext): Promise<string> {
	...
	const avatarUrl = safeHttpUrl((user as { avatar_url?: string }).avatar_url) ?? '/avatar.png';
...
export async function settingsAfter(_ctx: RouteContext): Promise<void> {
		...
		if (anonCheckbox.checked) {
			...
			if (f && f.type.startsWith('image/')) {
				...
				try {
					...
					imgPreview.src = parsedUrl.protocol === 'blob:' ? parsedUrl.href : originalAvatarUrl;
				} catch {
					imgPreview.src = originalAvatarUrl;
				}
			} else {
				imgPreview.src = originalAvatarUrl;
			}
	...
	avatarInput.addEventListener('change', (e) => {
		...
		if (!anonCheckbox.checked) imgPreview.src = parsedUrl.href;
	});
