...
function clothesCard(res: ClothesResource): string {
	...
	const imgHtml = res.thumbnail_media_uuid
		? `<div class="card-image">${progressiveImg({ uuid: res.thumbnail_media_uuid, placeholder: res.placeholder_blur, res: 'low', alt: title, processed: res.processed })}<span class="card-badge">${res.meta.clothing_type}</span></div>`
