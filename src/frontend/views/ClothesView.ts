...
function clothesCard(res: ClothesResource): string {
	...
	const imgHtml = res.thumbnail_key
		? `<div class="card-image"><img src="/media/${res.thumbnail_media_uuid}/thumbnail" alt="${title}" loading="lazy" decoding="async"><span class="card-badge">${res.meta.clothing_type}</span></div>`
