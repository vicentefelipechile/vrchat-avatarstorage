...
// ASSET ROUTES (v2)
// =========================================================================================================
// Thin HTTP handlers for the assets category. Handlers only: parse/validate input,
// call the service, shape the response, and manage HTTP-only concerns (KV cache).
// All business logic and SQL live in AssetService / AssetRepository.
//
...
assets.get('/', async (c) => {
	...
	try {
		const result = await new AssetService(c.env.DB).list(parsed.data);
		return c.json(result);
	} catch (e) {
		console.error('Assets list error:', e);
		return fail(c, 'Failed to fetch assets', 500);
	}
});
...
assets.get('/:uuid', async (c) => {
	try {
		const row = await new AssetService(c.env.DB).detail(c.req.param('uuid')!);
		return c.json(row);
	} catch (e) {
...
// =========================================================================================================
// POST /api/assets
// Create a resource + asset_meta (+ links + media) in one batch — no history entry on creation.
// =========================================================================================================
...
assets.post('/', requireAuth, async (c) => {
	...
	try {
		const { uuid } = await new AssetService(c.env.DB).create(c.get('user'), {
			title: d.title,
		...
		return fail(c, 'Failed to create asset', 500);
	}
});

// =========================================================================================================
// PUT /api/assets/:uuid
// Edit asset metadata — snapshot previous state, record meta_edit in history, update in one batch.
// =========================================================================================================

assets.put('/:uuid', requireAdmin, async (c) => {
	...
	try {
		await new AssetService(c.env.DB).update(c.get('user'), uuid, metaParsed.data);
