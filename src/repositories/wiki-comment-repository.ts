...
export class WikiCommentRepository {
	...
	findUserByUsername(username: string): Promise<{ uuid: string; avatar_url: string; is_anonymous: number } | null> {
		return queryOne<{ uuid: string; avatar_url: string; is_anonymous: number }>(
			this.db,
