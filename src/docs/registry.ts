...
export const ENDPOINTS: EndpointDoc[] = [
	// ---- Avatars (public reads + auth writes) -----------------------------------------------------------------
	{
		...
		description: 'Paginated avatar listing filtered by gender, size, type, platform, SDK, NSFW, feature flags, and author. Uses INNER JOIN avatar_meta — only avatars with metadata appear.',
		params: [
			...
			q('avatar_gender', 'Filter by gender', { type: 'enum', enumValues: ['male','female','androgynous','undefined','both'] }),
			q('avatar_size', 'Filter by size', { type: 'enum', enumValues: ['tiny','small','medium','tall','giant'] }),
			q('avatar_type', 'Filter by avatar type', { type: 'enum', enumValues: ['human','furry','anime','chibi','cartoon','semi-realistic','monster','fantasy','mecha','kemono','sci-fi','vtuber','other'] }),
			q('platform', 'Filter by platform', { type: 'enum', enumValues: ['pc','quest','cross'] }),
			q('sdk_version', 'Filter by SDK', { type: 'enum', enumValues: ['sdk3','sdk2'] }),
	...
	// ---- Assets -----------------------------------------------------------------------------------------------
	{
		...
		description: 'Paginated asset listing filtered by asset_type, platform, SDK, Unity version, and NSFW. INNER JOIN asset_meta.',
		params: [
			q('page', 'Page number', { type: 'integer', defaultValue: '1' }),
			q('limit', 'Items per page', { type: 'integer', defaultValue: '24', enumValues: ['1..60'] }),
			q('sort_by', 'Sort field', { type: 'enum', enumValues: ['created_at','download_count','title'], defaultValue: 'created_at' }),
			q('sort_order', 'Sort direction', { type: 'enum', enumValues: ['asc','desc'], defaultValue: 'desc' }),
			q('asset_type', 'Filter by asset type', { type: 'enum', enumValues: ['prop','shader','particle','vfx','prefab','script','animation','avatar-base','texture-pack','sound','tool','hud','other'] }),
			q('platform', 'Filter by platform', { type: 'enum', enumValues: ['pc','quest','cross'] }),
			q('sdk_version', 'Filter by SDK', { type: 'enum', enumValues: ['sdk3','sdk2'] }),
			q('unity_version', 'Filter by Unity version', { type: 'enum', enumValues: ['2019','2022'] }),
			q('is_nsfw', 'NSFW filter', { type: 'enum', enumValues: ['0','1'] }),
		],
	...
	// ---- Clothes ----------------------------------------------------------------------------------------------
	{
		...
		description: 'Paginated clothes listing filtered by gender_fit, clothing_type, platform, is_base, is_nsfw, and has_physbones. INNER JOIN clothes_meta.',
		params: [
			q('page', 'Page number', { type: 'integer', defaultValue: '1' }),
			q('limit', 'Items per page', { type: 'integer', defaultValue: '24', enumValues: ['1..60'] }),
			q('sort_by', 'Sort field', { type: 'enum', enumValues: ['created_at','download_count','title'], defaultValue: 'created_at' }),
			q('sort_order', 'Sort direction', { type: 'enum', enumValues: ['asc','desc'], defaultValue: 'desc' }),
			q('gender_fit', 'Filter by gender fit', { type: 'enum', enumValues: ['male','female','unisex','kemono'] }),
			q('clothing_type', 'Filter by clothing type', { type: 'enum', enumValues: ['top','jacket','bottom','dress','fullbody','swimwear','shoes','legwear','hat','hair','accessory','tail','ears','wings','body-part','underwear','other'] }),
			q('platform', 'Filter by platform', { type: 'enum', enumValues: ['pc','quest','cross'] }),
