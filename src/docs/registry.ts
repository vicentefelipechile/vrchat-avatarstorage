...
export const ENDPOINTS: EndpointDoc[] = [
	...
	// ---- Clothes ----------------------------------------------------------------------------------------------
	{
		...
		description: 'Paginated clothes listing filtered by title substring (q) plus gender_fit, clothing_type (multi-type, OR via junction clothes_clothing_types), platform, is_base, is_nsfw, and has_physbones. q is substring on resources.title via FTS5 resources_fts. Multi-value filters use repeated keys; a resource matches if it has ANY selected clothing_type.',
		params: [
			q('page', 'Page number', { type: 'integer', defaultValue: '1' }),
			q('limit', 'Items per page', { type: 'integer', defaultValue: '24', enumValues: ['1..60'] }),
			q('sort_by', 'Sort field', { type: 'enum', enumValues: ['created_at','download_count','title'], defaultValue: 'created_at' }),
			...
			q('clothing_type', 'Filter by clothing type — multi-type OR (repeat for OR; match if resource has ANY)', { type: 'enum', enumValues: ['top','jacket','bottom','dress','fullbody','swimwear','shoes','legwear','hat','hair','accessory','tail','ears','wings','body-part','underwear','other'] }),
			q('platform', 'Filter by platform (repeat for OR)', { type: 'enum', enumValues: ['pc','quest','cross'] }),
		...
		response: { description: 'JSON — { resources: ClothesWithMeta[] (meta.clothing_type is string[]), pagination }' },
	},
	{
		...
		description: 'Full clothing detail including clothes_meta + clothes_clothing_types (clothing_type is string[]).',
		params: [p('uuid', 'Clothes resource UUID')],
		response: { description: 'JSON — ClothesWithMeta (clothing_type: string[] 1..8)' },
	},
	{
		...
		description: 'Creates a resource + clothes_meta + clothes_clothing_types (1..8 clothing_type values) + links + media in one batch (inactive, pending approval). Turnstile required.',
		params: [b('title', 'Clothing title', { required: true }), b('thumbnail_uuid', 'Media UUID for thumbnail', { required: true }), b('meta', 'ClothesMetaSchema payload (clothing_type: string[] 1..8)', { required: true, type: 'object' })],
		response: { description: '201 — { uuid }' },
	},
	{
		...
		description: 'Snapshots previous clothes_meta + junction, records meta_edit, then updates. clothing_type is array 1..8 (junction replaced). Requires admin.',
		params: [p('uuid', 'Clothes resource UUID')],
		response: { description: 'JSON — { success: true }' },
	},
	// ---- Resources (generic) ----------------------------------------------------------------------------------
