...
window.approveResource = async (uuid) => {
    try {
...
window.rejectResource = async (uuid) => {
...
window.deactivateResource = async (uuid) => {
...
class ItemView extends AbstractView {
    async getHtml() {
        ...
        return `
            <div class="details-box">
                <h1>${res.title}${!res.is_active ? ` <span class="under-revision">(${t('item.underReview')})</span>` : ''}</h1>
                <div class="meta">
                ...
                <div id="admin-actions" style="margin-top: 30px;">
                    ${this.getAdminActions(res)}
                </div>
            </div>
        `;
    }
    ...
    getAdminActions(res) {
        ...
        if (!res.is_active || res.is_active === 0) {
            // Pending
            buttons = `
                <div style="background: #fff3cd; color: #856404; padding: 15px; border-radius: 5px; border: 1px solid #ffeeba; margin-top: 10px;">
                    <h3>${t('item.adminPanel')}</h3>
                    <p>${t('item.pendingApproval')}</p>
                    <button class="btn" style="background-color: #28a745;" onclick="approveResource('${res.uuid}')">${t('item.approve')}</button>
                    <button class="btn" style="background-color: #dc3545;" onclick="rejectResource('${res.uuid}')">${t('item.reject')}</button>
                </div>
            `;
        } else {
            // Active
            buttons = `
                <div style="background: #d1ecf1; color: #0c5460; padding: 15px; border-radius: 5px; border: 1px solid #bee5eb; margin-top: 10px;">
                    <h3>${t('item.adminPanel')}</h3>
                    <button class="btn" style="background-color: #ffc107; color: black;" onclick="deactivateResource('${res.uuid}')">${t('item.deactivate')}</button>
                </div>
            `;
        }
        return buttons;
    ...
    async postRender() {
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (avatarFile) {
                    const resizedAvatar = await resizeImage(avatarFile, 64, 64);
