...
- **Contacts:** To detect collisions (like in [SPS](/wiki?topic=sps) or [PCS](/wiki?topic=pcs) systems).
...
1.  **Parameter Exhaustion:**
    ...
    - _Consequence:_ VRChat will block the avatar upload or the last installed components will not work.
...
- **Inefficient Way (Bools):** You create 10 `Bool` parameters (Shirt1, Shirt2... Shirt10).
  - _Cost:_ 10 Bits.
  - _Disadvantage:_ You spend 1 bit for each extra garment.
- **Efficient Way (Int):** You create **1** single `Int` parameter called `Top_Clothing`.
  - _Cost:_ 8 Bits (always, since it's an Int).
  - _Advantage:_ You can have up to **255 shirts** using the same 8 bits!
  - _How it works:_ In the Animator, you set it so if the value is 1, Shirt A activates; if it's 2, Shirt B, etc.
