...
> [!WARNING]
> Attenzione
> Per questo passaggio, devi usare il **disinstallatore ufficiale di Windows** (da _Impostazioni -> App_ o _Pannello di Controllo_). **NON usare programmi di terze parti** come IObit Uninstaller, Revo Uninstaller, ecc., poiché possono eliminare chiavi di registro necessarie e peggiorare il problema.
...
_(Nota: puoi copiare e incollare il percorso `%USERPROFILE%` direttamente nella barra dell'explorer, proprio come faresti con `%appdata%` per installare mod di Minecraft, e ti porterà automaticamente alla cartella dell'utente corrente)._
