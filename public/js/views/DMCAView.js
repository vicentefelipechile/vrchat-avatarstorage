...
export default class DMCAView extends AbstractView {
    constructor(params) {
        super(params);
        this.setTitle('VRCStorage - DMCA');
    }
        ...
        <div class="dmca-page">
            ...
            <div class="dmca-panel" id="dmca-panel-simple">
                <div class="dmca-story-builder">
                    ...
                    <!-- Company name (representative only) -->
                    ...
                    <!-- Third reporter: reason (preset dropdown + optional other) -->
                    ...
                    <!-- What right is infringed? (hidden for third_reporter) -->
                    ...
                    <!-- Original owner name (third-party rights only) -->
                    ...
                    <!-- Proof of authenticity (author / representative only) -->
                    ...
                    <button class="btn dmca-send-btn" id="s-send-btn">
                        ${t('dmca.sendEmail')}
                    </button>
                </div>
            </div>
            ...
            <div class="dmca-panel" id="dmca-panel-advanced" style="display:none">
                <div class="dmca-advanced-builder">
                    ...
                    <button class="btn dmca-send-btn" id="a-send-btn">
                        ${t('dmca.sendEmail')}
                    </button>
                </div>
            </div>
    ...
    async postRender() {
        // Tab switching
        document.querySelectorAll('.dmca-tab-btn').forEach(btn => {
        ...
        // Simple mode: reactive dropdowns
        const claimantType = document.getElementById('s-claimant-type');
        ...
        const infringementType = document.getElementById('s-infringement-type');
        ...
        const updateSimpleVisibility = () => {
            ...
            // Company row: only for representative
            companyRow.style.display = ct === 'representative' ? 'flex' : 'none';

            // Third reporter specific fields
            reporterNameRow.style.display = isThird ? 'flex' : 'none';
            reporterReasonGroup.style.display = isThird ? '' : 'none';

            // Infringement row: hidden for pure third-party reporters
            infringementRow.style.display = isThird ? 'none' : '';

            // Owner name: only when infringement type is "third" AND not a third_reporter
            ownerRow.style.display = (!isThird && it === 'third') ? 'flex' : 'none';

            // Proof of authenticity: only meaningful for author / representative
        ...
        ['s-company-name', 's-owner-name', 's-resource-urls', 's-proof-text',
            's-action-type', 's-reporter-name', 's-reporter-reason', 's-reporter-reason-other'].forEach(id => {
                const el = document.getElementById(id);
                if (el) el.addEventListener('input', updatePreview);
                if (el) el.addEventListener('change', updatePreview);
            });

        // Show/hide "other" textarea when reason dropdown changes
        const reasonSelect = document.getElementById('s-reporter-reason');
        const reasonOther = document.getElementById('s-reporter-reason-other');
        if (reasonSelect) {
        ...
        document.getElementById('s-send-btn').addEventListener('click', () => {
            const body = buildSimpleBody();
            const subject = encodeURIComponent('DMCA Takedown Notice - VRCStorage');
            const bodyEnc = encodeURIComponent(body);
            window.location.href = `mailto:dmca@vrcstorage.lat?subject=${subject}&body=${bodyEnc}`;
        });

        // Advanced send button
        document.getElementById('a-send-btn').addEventListener('click', () => {
            const claim = document.getElementById('a-claim-text').value.trim();
            const resources = document.getElementById('a-resources-text').value.trim();
            const evidence = document.getElementById('a-evidence-text').value.trim();
        ...
        function updatePreview() {
            const previewEl = document.getElementById('s-preview-text');
            if (!previewEl) return;
            previewEl.textContent = buildSimpleBody();
        }

        function buildSimpleBody() {
            const ct = document.getElementById('s-claimant-type')?.value;
            const companyName = document.getElementById('s-company-name')?.value?.trim() || '___';
            const it = document.getElementById('s-infringement-type')?.value;
            const ownerName = document.getElementById('s-owner-name')?.value?.trim() || '___';
            const action = document.getElementById('s-action-type')?.value;
            const urlsRaw = document.getElementById('s-resource-urls')?.value?.trim() || '';
            const proof = document.getElementById('s-proof-text')?.value?.trim() || '';
            const reporterName = document.getElementById('s-reporter-name')?.value?.trim() || '___';
            const reasonSelect = document.getElementById('s-reporter-reason');
            const reasonOtherEl = document.getElementById('s-reporter-reason-other');
            const reporterReason = reasonSelect?.value === 'other'
                ? (reasonOtherEl?.value?.trim() || '___')
                : (() => {
                    const map = {
                    ...
                    return map[reasonSelect?.value] || '___';
                })();
            ...
            let claimantStr;
            if (ct === 'author') {
                claimantStr = t('dmca.simple.optAuthor');
            } else if (ct === 'representative') {
                claimantStr = `${t('dmca.simple.optRepresentative')} ${t('dmca.simple.of')} ${companyName}`;
            } else {
                claimantStr = `${t('dmca.simple.optThirdReporter')} (${reporterName})`;
            }
            ...
            if (infringementStr) {
                ...
                body += `${t('dmca.simple.bodyThirdReporterReason')}: ${reporterReason}\n\n`;
            }

            body += `${t('dmca.simple.bodyDemand')} ${actionStr}.\n\n`;

            if (proof && ct !== 'third_reporter') {
                body += `${t('dmca.simple.bodyProof')}:\n${proof}\n\n`;
            }

            body +=
                `${t('dmca.simple.bodyDeclaration')}\n\n` +
